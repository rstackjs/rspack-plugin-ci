...
import { describe, expect, it, afterAll } from 'rstack/test';
        ...
        if (/^MISSING/.test(content)) {
          ...
          if (UPDATE_TEST) {
            ...
            if (match) {
              ...
              // update expected
              const newExpectedPath = removeHash(
                path.resolve(expected, realFile),
              );
              fs.writeFileSync(
                newExpectedPath,
    ...
    if (!/^(\.|_)/.test(directory)) {
      ...
      it(`${directory} should compile to the expected result`, async () => {
        if (directory === 'serializingBigStrings') {
          clearDirectory(
            path.resolve(import.meta.dirname, '../node_modules/.cache'),
          );
        }
