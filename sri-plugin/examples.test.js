...
import { describe, rstest, test } from 'rstack/test';
...
const DISABLED_RSPACK_CASES = [...DISABLED_CASES];

const exampleDir = join(__dirname, 'examples');
const rspackCliBin = join(
  __dirname,
...
function createTestCases(type) {
  readdirSync(exampleDir)
    .filter(
      (i) =>
        !(type === 'rspack' ? DISABLED_RSPACK_CASES : DISABLED_CASES).includes(
          i,
        ),
    )
    .forEach((example) => {
